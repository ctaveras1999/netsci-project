...
def hgraph_from_lists(edge_list, triangle_list):
    h_graph = {}

    for i in range(len(edge_list)):
        u, v = edge_list.iloc[i][0], edge_list.iloc[i][1]
        u, v = np.sort([u, v])
        label = str(u) + "_" + str(v)
        h_graph[label] = (u, v)

    for i in range(len(triangle_list)):
        u, v, w = triangle_list.iloc[i][0], triangle_list.iloc[i][1], triangle_list.iloc[i][2]
        u, v, w = np.sort([u, v, w])
        label = str(u) + "_" + str(v) + "_" + str(w)
        h_graph[label] = (u, v, w)

    return hnx.Hypergraph(h_graph)

def hgraph_from_graph(G, triangle_list):
    h_graph = {}
...
def hgraph_to_sc(h_graph):
    edge_list = []
    triangle_list = []
    for edge in h_graph.edges:
        node_list = [int(node) for node in edge.split("_")]
        if len(node_list) > 2:
            for subset in itertools.combinations(node_list, 2):
                edge_list.append(subset)
        if len(node_list) > 3:
            for subset in itertools.combinations(node_list, 3):
                triangle_list.append(subset)
    return edge_list, triangle_list

def get_dblp_data(file_name):
    H, truth, partitions = pickle.load(open(file_name,'rb'))
    h_graph = {}
    n_edge = 0 
    for i, e in enumerate(H):
        for edge in e:
            sorted_edge = sorted(list(edge))
            n_edge += 1
            edge_name = ""
            for j, node in enumerate(sorted_edge):
                if j > 0:
                    edge_name += "_"
                edge_name += str(node)
            h_graph[edge_name] = sorted_edge

    h_graph = hnx.Hypergraph(h_graph)
    return h_graph, truth, partitions


def compute_modularity(HG, part, wdc):
    higher_part = hmod.last_step(HG, part, wdc)
    higher_mod = hmod.modularity(HG, higher_part, wdc)
    return higher_mod
    
def edge_comm_to_node_comm(G, edge_class):
    node_class = dict.fromkeys(G.nodes(), 0)   
    all_count = Counter(edge_class)
    comm = [k for k in all_count if all_count[k] >= 1]

    for n in list(G.nodes):
        
        n_count = dict.fromkeys(all_count.keys(), 0)    
        
        for index, (u,v) in enumerate(G.edges):
            
            if n == u or n == v:
                label = edge_class[index]
                if label in comm:
                    n_count[label] += 1
        
        node_class[n] = max(n_count, key=n_count.get)
